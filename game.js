async function readFile(file) {
    try {
        ...
        $('.node-text').append('<p>Error: ' + error.message + '</p>');async function readFile(file) {
    try {
        const response = await fetch(file);
        if (!response.ok) {
            throw new Error('Network response was not ok');
...
async function main() {
    ...
    while (true) {
        ...
        const feedbackLenght = currentOption.feedback.length;
        ...
        await showFeedback(feedbackLenght, data.pauseTime);
...
async function main() {
    ...
    while (true) {
        ...
        await showFeedback(feedbackLenght, PAUSETIME);
